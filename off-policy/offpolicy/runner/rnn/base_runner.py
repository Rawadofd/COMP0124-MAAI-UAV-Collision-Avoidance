...
class RecRunner(object):
    ...
    def __init__(self, config):
        ...
        self.share_policy = True #self.args.share_policy
        self.algorithm_name = self.args.algorithm_name
        self.env_name = self.args.env_name
        self.num_env_steps = self.args.num_env_steps
        self.use_wandb = False #self.args.use_wandb
    ...
    def run(self):
        ...
        env_info, act_info = self.collecter(explore=True, training_episode=True, warmup=False)
        for k, v in env_info.items():
            self.env_infos[k].append(v)
        # ================================================ #
        # Store the actions into the dict
        for k, v in act_info.items():
            self.env_infos[k].append(v)
            #print(self.env_infos)
    ...
    def warmup(self, num_warmup_episodes):
        ...
        for _ in range((num_warmup_episodes // self.num_envs) + 1):
            env_info, act_info = self.collecter(explore=True, training_episode=False, warmup=True)
