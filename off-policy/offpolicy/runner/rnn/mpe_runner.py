...
class MPERunner(RecRunner):
    ...
    def eval(self):
        ...
        eval_infos['episode_acts'] = []

        for _ in range(self.args.num_eval_episodes):
            env_info, act_info = self.collecter( explore=False, training_episode=False, warmup=False)
            for k, v in env_info.items():
                eval_infos[k].append(v)
            # ==================== Store the actions in each episode =================== #
            ...
            #     print('NOOOOO!!!!')
            # else:
            #     print('num_step: ', act_info['episode_acts'])
            # for k, v in act_info.items():
            #     eval_infos[k].append(v)
    ...
    def shared_collect_rollout(self, explore=True, training_episode=True, warmup=False):
        ...
        total_acts = {p_id: np.zeros((self.num_agents, policy.output_dim))}
        ...
        while t < self.episode_length:
            ...
            total_acts[p_id] = np.append(total_acts[p_id],np.squeeze(np.array(env_acts)),axis=0)
        ...
        act_info['episode_acts'] = episode_acts[p_id]
        

        return env_info, act_info
